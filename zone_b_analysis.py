...
확정 수치:
    단천항 5km 가용지(1단계): 11.44 km²  (Zone B+C+E 배치 1.85 km²)
    AOI 전체 가용지:          121.83 km²
    ※ 수치 정정 이력은 README.md 및 04_flood_risk_analysis.js 참조
      (v1 "2.19 km²"는 unmask 누락 오류 → v2 보정)
...
jrc_water = (
    ...
    .select('occurrence')
    .clip(cmez_roi)
    .unmask(0)        # ★ v1 오류 수정: 수면 마스크 unmask 누락 보정
)
...
flood_hazard = permanent_water.focal_max(
    radius=500,
    units='meters'
).unmask(0)       # ★ v1 오류 수정 지점

# ─────────────────────────────────────────
# 6. 가용지 + 단천항 5km 1단계 구역 산출
#    조건: 경사도 5도 이하 AND 홍수 위험 제외
# ─────────────────────────────────────────

# 가용지: 경사<5° AND 홍수 위험 제외
available = suitable_slope.And(flood_hazard.Not())

# 1단계 구역: 가용지 AND 단천항 5km 배후권
port_pt = ee.Geometry.Point(PORT)
port_zone = ee.Image(0).paint(
    ee.FeatureCollection([ee.Feature(port_pt.buffer(5000))]), 1
).clip(cmez_roi)
phase1_zone = available.And(port_zone)
...
calc_area_km2(available,   cmez_roi, "AOI 전체 가용지")
calc_area_km2(phase1_zone, cmez_roi, "★ 단천항 5km 가용지(1단계)")
...
export_task = ee.batch.Export.image.toDrive(
    image=ee.Image([
        ...
        phase1_zone.toFloat().rename('phase1_port5km'),
